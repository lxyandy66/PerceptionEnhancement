"""
策略3: 时间切换冷热两态 — 10:00-17:00 热态(Hot), 其余时间冷态(Cold)
"""
import csv, math, os
...
CONSTRUCTION_OFFSET = 30  # PNIPAM Step 0 在 Denver IDF 中的全局索引 (Step0=Hot=30, Step24=Cold=54)
...
class TimeSwitch(EnergyPlusPlugin):
    def __init__(self):
        super().__init__()
    ...
    def _load_csv(self):
        ...
        print(f"[TimeSwitch] Loaded {self._csv_count} rows")
    ...
    def _ensure_handles(self,state):
        ...
        for name in FEN_NAMES:
            ...
            self._h_cs.append(h)
        self._h_ready=True
        print(f"[TimeSwitch] Handles: weather OK, cs={len(self._h_cs)}")
    ...
    def on_end_of_zone_timestep_after_zone_reporting(self,state)->int:
        ...
        if not self._warmup_done:
            return 0
        _, _, _, h, _ = self._sim_datetime(self._timestep_idx)
        # 10:00-17:00 -> Hot(idx=0), 其余 -> Cold(idx=24)
        state_idx = 0 if (10 <= h < 17) else 24
        self._next_state_idx = state_idx + CONSTRUCTION_OFFSET
        return 0
