...
// 加热器引脚
#define PIN_ENABLE_HEATING D3
// 电阻间断测量引脚
#define PIN_ENABLE_MEASUREMENT D13
...
void setup() {
  ...
  pinMode(PIN_ENABLE_MEASUREMENT, OUTPUT);
  pinMode(PIN_ENABLE_HEATING, OUTPUT);
  ...
  digitalWrite(PIN_ENABLE_HEATING, isHeating?HIGH:LOW);//不使能读取
  digitalWrite(PIN_ENABLE_MEASUREMENT, SWITCH_DISABLE);//不使能读取
...
void loop() {
  if (sampleChrono.hasPassed(SAMPLING_INTERVAL)) {
    ...
    /**** 使能读取 ****/
    digitalWrite(PIN_ENABLE_MEASUREMENT, SWITCH_ENABLE);  // 串联继电器连通 使能
    delay(TIME_STARTUP);
    // 读取部分
