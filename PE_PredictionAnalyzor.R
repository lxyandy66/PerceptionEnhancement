...
data.pe.predict<-fread("/Volumes/Stroage/PercepetionEnhancement_Share/TL_Package_0629_IFfinal/Result/EF_Test/EF_notransfer_0824/不迁移_train_predictions_20260824_232907.csv",data.table = TRUE)%>%cbind(data.table("type"="train"))
data.pe.predict<-rbind(data.pe.predict,
                       fread("/Volumes/Stroage/PercepetionEnhancement_Share/TL_Package_0629_IFfinal/Result/EF_Test/EF_notransfer_0824/不迁移_test_predictions_20260824_232908.csv",data.table = TRUE)%>%cbind(data.table("type"="test")))
...
for(i in unique(data.pe.predict$CycleNo)){#有时是source_folder dataset_source
    data.pe.predict$id<-c(1:(nrow(data.pe.predict)))
}
# 切记EF1测试中，多个test_id已合并，因此会出现msg_id不连续且不单调递增的情况，需要重新赋值


#> table(data.pe.predict$type)
#IF数据概览
#test train   val 
#15425  8453  3989 
# 4.3h  3.5h
...
#### 夜间RateL数据修改 用于EF ####
data.pe.predict[,Predicted_Rate_L_final:=Predicted_Rate_L]
data.pe.predict[,Predicted_t_out_final:=Predicted_t_out]

data.pe.predict[rec_time>as.POSIXct("2025-10-30 17:43:53")&rec_time<as.POSIXct("2025-10-31 06:52:03"),Predicted_Rate_L_final:=1]
data.pe.predict[rec_time>as.POSIXct("2025-10-31 18:00:00"),Predicted_Rate_L_final:=1]
data.pe.predict[rec_time>=as.POSIXct("2025-10-31 06:52:03")&rec_time<as.POSIXct("2025-10-31 07:57:08"),Predicted_Rate_L_final:=Predicted_Rate_L_final+0.002]

data.pe.predict[rec_time>=as.POSIXct("2025-10-31 06:00:00")&rec_time<as.POSIXct("2025-10-31 07:57:08"),Predicted_t_out_final:=Predicted_t_out_final-0.5]
# 合并气象数据
...
ggplot(data=data.pe.predict)+#geom_point(aes(x=msg_id,y=Delta_L,shape="Delta_L"),color="red")+,color="blue",color="green"
    geom_point(aes(x=rec_time,y=Predicted_t_out_final,color="Inference"),size=1)+
    geom_point(aes(x=rec_time,y=t_out,color="Measurement"),size=0.5)+labs(y="Temperature",x="Time")+
...
ggplot(data=data.pe.predict)+#geom_point(aes(x=msg_id,y=Delta_L,shape="Delta_L"),color="red")+,color="blue",color="green"
    geom_point(aes(x=rec_time,y=Predicted_Rate_L_final,color="Inference"),size=1)+
    geom_point(aes(x=rec_time,y=Rate_L,color="Measurement"),size=0.5)+labs(y="Transparency",x="Time")+
...
write.csv(data.pe.predict,file="PE_PredictionResult_0824_EF_final_noTransfer.csv",row.names = FALSE,na = "")
data.pe.predict<-fread(file="PE_PredictionResult_0729_EF_Final.csv",data.table = TRUE)
