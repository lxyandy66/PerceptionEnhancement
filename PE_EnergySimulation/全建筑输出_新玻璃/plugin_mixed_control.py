"""
策略3: 南窗 CSV Rate→25态, 其他窗玻璃温度→25态
"""
import csv, math, os
...
CONSTRUCTION_OFFSET = 32  # PNIPAM Step 0 = 全局索引 32  # PNIPAM Step 0 的全局构造索引（三明治结构IDF）

SOUTH = ["PERIMETER_BOT_ZN_1_WALL_SOUTH_WINDOW1","PERIMETER_BOT_ZN_1_WALL_SOUTH_WINDOW2",
         "PERIMETER_BOT_ZN_1_WALL_SOUTH_WINDOW3","PERIMETER_MID_ZN_1_WALL_SOUTH_WINDOW",
         "PERIMETER_TOP_ZN_1_WALL_SOUTH_WINDOW"]
OTHER = ["PERIMETER_BOT_ZN_3_WALL_NORTH_WINDOW1","PERIMETER_BOT_ZN_3_WALL_NORTH_WINDOW2",
         "PERIMETER_BOT_ZN_3_WALL_NORTH_WINDOW3","PERIMETER_BOT_ZN_2_WALL_EAST_WINDOW1",
         "PERIMETER_BOT_ZN_2_WALL_EAST_WINDOW2","PERIMETER_BOT_ZN_4_WALL_WEST_WINDOW1",
         "PERIMETER_BOT_ZN_4_WALL_WEST_WINDOW2","PERIMETER_MID_ZN_2_WALL_EAST_WINDOW",
         "PERIMETER_MID_ZN_3_WALL_NORTH_WINDOW","PERIMETER_MID_ZN_4_WALL_WEST_WINDOW",
         "PERIMETER_TOP_ZN_2_WALL_EAST_WINDOW","PERIMETER_TOP_ZN_3_WALL_NORTH_WINDOW",
         "PERIMETER_TOP_ZN_4_WALL_WEST_WINDOW"]
ALL_WINDOWS = SOUTH + OTHER  # 南窗在前5，其他在后13
...
def tsol_to_step(tsol):
    ...
    return int(round(max(0.0, min(1.0, frac)) * N_STEPS))

class MixedControl(EnergyPlusPlugin):
    def __init__(self):
        super().__init__()
        ...
        self._h_cs_south=[]; self._h_cs_other=[]; self._h_tw_other=[]
    ...
    def _load_csv(self):
        ...
        self._csv_count=len(self._csv_data); print(f"[Mixed] Loaded {self._csv_count} rows")

    def _csv_row(self,sim_total_min):
        if self._csv_count==0: return {"t_env":25,"hum":50,"wind":0,"rad":0,"rate":0.5}
    ...
    def _ensure_handles(self,state):
        ...
        for name in SOUTH:
            self._h_cs_south.append(ex.get_actuator_handle(state,"Surface","Construction State",name))
        for name in OTHER:
            self._h_cs_other.append(ex.get_actuator_handle(state,"Surface","Construction State",name))
            self._h_tw_other.append(ex.get_variable_handle(state,"Surface Outside Face Temperature",name))
        self._h_ready=True
        print(f"[Mixed] Handles: weather OK, south_cs={len(self._h_cs_south)}, other_cs={len(self._h_cs_other)}, other_tw={len(self._h_tw_other)}")

    def on_after_new_environment_warmup_is_complete(self,state)->int:
        self._warmup_done=True; self._timestep_idx=0; self._next_idxs=[N_STEPS]*18; return 0

    def on_begin_zone_timestep_before_set_current_weather(self,state)->int:
        self._ensure_handles(state); ex=self.api.exchange
        if not self._warmup_done:
            row=self._csv_data[0] if self._csv_count else {"t_env":25,"hum":50,"wind":0,"rad":0,"rate":0.5}
        else:
            row=self._csv_row(self._timestep_idx); self._timestep_idx+=1
        ex.set_actuator_value(state,self._h_env,row["t_env"]); ex.set_actuator_value(state,self._h_wind,row["wind"])
        ...
        # Apply stored indices - south windows first (indices 0-4), then other (5-17)
        all_cs = self._h_cs_south + self._h_cs_other
        for i, h_cs in enumerate(all_cs):
    ...
    def on_end_of_zone_timestep_after_zone_reporting(self,state)->int:
        ...
        ex=self.api.exchange
        # South windows: CSV Rate
        row = self._csv_row(self._timestep_idx)
        rate = row["rate"]
        tsol_target = TSOL_HOT + (TSOL_COLD - TSOL_HOT) * rate
        self._next_idxs[0] = self._next_idxs[1] = self._next_idxs[2] = self._next_idxs[3] = self._next_idxs[4] = tsol_to_step(tsol_target)
        # Other windows: glass temp
        for i in range(min(len(self._h_tw_other), 13)):
            if self._h_tw_other[i] >= 0:
                tw = ex.get_variable_value(state, self._h_tw_other[i])
                self._next_idxs[5+i] = tsol_to_step(tsol_from_glass_temp(tw))
        return 0
